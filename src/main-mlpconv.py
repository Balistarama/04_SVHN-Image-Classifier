...
def conv2d(x, W):
  ...
  conv_x = tf.nn.conv2d(input=x, filter=W, strides=[1, 1, 1, 1], padding='SAME')

  image_width = x.get_shape()
  W_shape = W.get_shape()

  W_fc = weight_variable([image_width[1] * image_width[1] * W_shape[3], 256])
  b_fc = bias_variable([256])

  conv_x_flat = tf.reshape(conv_x, [-1, image_width[1] * image_width[1] * W_shape[3]])
  mlpconv_x = tf.nn.relu6(tf.matmul(conv_x_flat, W_fc) + b_fc)
  mlpconv_x_unflat = tf.reshape(mlpconv_x, [CONVOLUTION_SIZE, CONVOLUTION_SIZE, W_shape[2], W_shape[3]])
